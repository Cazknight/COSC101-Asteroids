...
class Level_Manager
{
  ...
  boolean NewGame(Asteroid_Manager AM)
  {
    ...
    background(0);
    AM.UpdateAsteroids();
    image(title, (width - title.width) / 2, 100);
  ...
  boolean GameOver(String name, boolean acceptName, StringList Names, IntList Scores)
  {
    if(!acceptName)
    {
      ...
      stroke(255);
      fill(255);
      image(gameOver, (width - gameOver.width) / 2, 100);
      image(enterName, (width - enterName.width) / 2, 200);
    ...
    else
    {
      ...
      int offset = 0;
      background(0);
      image(highscoreTitle, (width - highscoreTitle.width) / 2, 20); 
      image(nameScore, (width - nameScore.width) / 2, 120); 
  ...
  void CenteredImageButton(PImage buttonImg, float buttonWidth, float buttonHeight, float yOffset, boolean onHover)
  {
    ...
    float imageXoffset = (buttonWidth - buttonImg.width) / 2;
    float imageYoffset = (buttonHeight - buttonImg.height) / 2;
    if(!onHover)
    {
      fill(255);
    }
    ...
    image(buttonImg, posX + imageXoffset , posY + imageYoffset);
  }
}